...
#include "qpmad/input_parser.h"
#include "qpmad/inverse.h"
#include "qpmad/solver_parameters.h"
#include "qpmad/constraint_status.h"
#include "qpmad/chosen_constraint.h"
#include "qpmad/active_set.h"
#include "qpmad/factorization_data.h"
    ...
    public:
                ...
                const SolverParameters &param)
        {
            ...
            switch (param.hessian_type_)
            {
                ...
                default:
                    throw("Malformed solver parameters!");
                    break;
            }
            ...
            for (MatrixIndex i = 0; i < num_constraints_; ++i)
            {
                ...
                if (lb_i - param.tolerance_ > ub_i)
                {
                    ...
                    throw("Inconsistent constraints (lb > ub).");
                }
                ...
                else
                {
                    ...
                    if (active_set_.hasEmptySpace())
                    {
                        ...
                        if (ctr_i_dot_primal_step_direction < -param.tolerance_)
                        {
                            ...
                            {
                                throw("Failed to add an equality constraint -- is this possible?");
                            }
                    ...
                    if (std::abs(chosen_ctr_.violation_) > param.tolerance_)
                    {
                        // nope it is not
                        constraints_status_[i] = ConstraintStatus::INCONSISTENT;
                        throw("Infeasible equality constraints");
                    }
            ...
            else
            {
                ...
                for (iter_counter_ = 1; (param.max_iter_ < 0) or (iter_counter_ <= param.max_iter_); ++iter_counter_)
                {
                        ...
                        else
                        {
                            ...
                            if (false
                                == factorization_data_.update(
                                        active_set_.size_, chosen_ctr_.is_simple_, param.tolerance_))
                            {
                                throw("Failed to add an inequality constraint -- is this possible?");
                            }
                    ...
                    else
                    {
                        ...
                        {
                            throw("Infeasible inequality constraints.");
                        }
                        else
                        {
                            QPMAD_TRACE("======================");
